...
namespace Tempo2012.UI.WPF.ViewModels
{
    public class LookupManagerViewModel:BaseViewModel,IReportBuilder
    {
        ...
        public LookupManagerViewModel(int codelookup){
            this._Lookups = new ObservableCollection<LookUpMetaData>(Context.GetAllLookups(""));
            if (this._Lookups != null)
            {
        ...
        public LookUpMetaData Lookup
        {
            ...
            set
            {
                ...
                LookupModelm.Fields.Add(new TableField{Name = "Фирма Код",
                                         DbField = "integer",
                                         NameEng = "FIRMAID"
                });
                CalculateFields();
        ...
        public void TransverLookUp()
        {
            ...
            foreach (var item in rez)
            {
                ...
                string command = string.Format("Insert into \"{0}\"({1}) VALUES({2});",Lookup.Tablename,keys,vals);
                sb.AppendLine(command);
            }
            string path=Path.Combine(Entrence.CurrentFirmaPathReport,Lookup.Tablename+".txt");
        ...
        {
            return Context.FbBatchExecution(string.Format("Delete from \"{0}\"",Lookup.Tablename));
        }

        public string DeleteAllItemsFirma()
        {
            return Context.FbBatchExecution(string.Format("Delete from \"{0}\" where FIRMAID={1}", Lookup.Tablename, ConfigTempoSinglenton.GetInstance().CurrentFirma.Id));
        }
