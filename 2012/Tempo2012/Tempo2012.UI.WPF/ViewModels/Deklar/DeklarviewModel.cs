        ...
        public static string ReturnViesDeclar(Dictionary<string, string> declar, List<ViesRow> list, List<ViesRowG> listg)
        {
          ...
          sb.AppendFormat("| Общ брой редове в раздел \"В\" на декларацията                    |{0,22}| 02|", declar["count"]);sb.AppendLine();
            ...
            if (listg.Count > 0)
            {
                ...
                sb.AppendLine("|на    | получателя/      |Код на операцията   | лицето, за което     |    през който е      |");
                sb.AppendLine("|ред   |придобиващия      |по режим складиране | стоките са били      |     извършена        |");
                sb.AppendLine("|      | (вкл.знака)      |стоки до поискване* | предназначени при    |    операцията по     |");
                sb.AppendLine("|      |                  |                    |заместване по чл. 15а,|   режим складиране   |");
                sb.AppendLine("|      |                  |                    |  тристранна          |     на стоки до      |");
                sb.AppendLine("|      |                  |                    |  операция            |      поискване       |");
                sb.AppendLine("|      |                  |                    |                      |    (мм/гггг)**       |");
                sb.AppendLine("+------+------------------+--------------------+----------------------+----------------------+");
