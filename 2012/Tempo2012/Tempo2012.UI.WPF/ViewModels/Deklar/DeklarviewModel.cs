...
namespace Tempo2012.UI.WPF.ViewModels.Deklar
{
    public class DeklarviewModel
    {
        public static string ReturnDeklar(Dictionary<string, string> declar)
        {
            ...
            sb.AppendLine();
            sb.AppendLine(" чл. 82, ал.2-6 ЗДДС             +---------------+--+ дост.по чл.82,ал. 2-5 ЗДДС     +---------------+--+");
            sb.AppendLine("                                                                                     +---------------+--+");
            ...
            sb.AppendLine();
            sb.AppendLine(" доставки по чл.82,ал.2-6         +--------------+--+");
            ...
            sb.AppendLine(" чл.82,ал.2-6 ЗДДС,вноса,както и");
        ...
        public static string ReturnViesDeclar(Dictionary<string, string> declar, List<ViesRow> list, List<ViesRowG> listg)
        {
            StringBuilder sb=new StringBuilder();
          ...
          sb.AppendFormat("| данъчна основа на ВОД                                           |{0,22}|   |",declar["sumak3"]);sb.AppendLine();
            ...
            sb.AppendLine("+-----------------------------------------------------------------+----------------------+---+");
          sb.AppendFormat("| Общ брой редове в раздел \"В\" на декларацията                    |{0,22}| 02|", declar["count"]);sb.AppendLine();
            sb.AppendLine("+-----------------------------------------------------------------+----------------------+---+");
            sb.AppendLine("+---+---------------+--------------------------------------------------------+---------------+");
            ...
            if (listg.Count > 0)
            {
                //Секция Г
                sb.AppendLine("---------------------------------------------------------------------------------------------+");
                sb.AppendLine("|Г. Прехвърляне на стоки от територията на страната до територията на друга държава членка   |");
                sb.AppendLine("|под режим складиране на стоки до поискване по чл. 15а, ал. 1 от ЗДДС, настъпила промяна     |");
                sb.AppendLine("|по чл. 15а, ал. 4 и 5 от ЗДДС или корекция на грешно посочен идентификационен  номер по ДДС |");
                sb.AppendLine("|на лицето, за което стоките са предназначени, в раздел \"Г\" на предходна декларация          |");
                sb.AppendLine("+--------------------------------------------------------------------------------------------+");
                sb.AppendFormat("| Общ брой редове в раздел \"Г\" на Декларацията                        |{0,18}| 02|", declar["countG"]); sb.AppendLine();
                sb.AppendLine("+------+------------------+------------------- +----------------------+----------------------+");
                sb.AppendLine("|No    | ИН по ДДС на     |                    |  ИН по ДДС на        |   Данъчен период,    |");
                sb.AppendLine("|на    | лицето, за което |Код на операцията   | лицето, за което     |    през който е      |");
                sb.AppendLine("|ред   |   стоките са     |по режим складиране | стоките са били      |     извършена        |");
                sb.AppendLine("|      |  предназначени   |стоки до поискване* | предназначени при    |    операцията по     |");
                sb.AppendLine("|      |   (вкл.знака)    |                    |заместване по чл. 15а,|   режим складиране   |");
                sb.AppendLine("|      |                  |                    |    ал. 4 от ЗДДС     |     на стоки до      |");
                sb.AppendLine("|      |                  |                    |    (вкл. знака)      |      поискване       |");
                sb.AppendLine("|      |                  |                    |                      |    (мм/гггг)**       |");
                sb.AppendLine("+------+------------------+--------------------+----------------------+----------------------+");
                sb.AppendLine("| к1   |      к2          |        к3          |       к4             |        к5            |");
                sb.AppendLine("+------+------------------+--------------------+----------------------+----------------------+");
                foreach (var l in listg)
                {
                    sb.AppendFormat("|{0,6}|{1,-18}|{2,20}|{3,22}|{4,22}|", l.NomRow, l.VIN, l.KOD, l.VINDest, l.PeriodOP);
                    sb.AppendLine();
                }
                sb.AppendLine("+------+------------------+--------------------+--------------------+------------------------+");

                sb.AppendLine("*В колона к. 3 се посочва код на операцията 1, 2 или 3 със следното значение:");
                sb.AppendLine("1 - изпращане или транспортиране на стоки от територията на страната до територията на друга ");
                sb.AppendLine("държава членка под режим складиране на стоки до поискване по чл. 15а, ал. 1 от ЗДДС");
                sb.AppendLine("2 - връщане по чл. 15а, ал. 5 от ЗДДС на територията на страната на стоки, изпратени или ");
                sb.AppendLine("транспортирани под режим складиране на стоки до поискване, или корекция на грешно посочен");
                sb.AppendLine("идентификационен номер по ДДС на лицето, за което стоките са предназначени, деклариран в ");
                sb.AppendLine("предходен данъчен период на изпращане или транспортиране на стоки по чл. 15а, ал. 1 от закона.");
                sb.AppendLine("В случай на връщане в колона 2 се посочва идентификационният номер по ДДС на лицето, за което ");
                sb.AppendLine("стоките са били предназначени към момента на връщането.В случай на корекция на грешно посочен ");
                sb.AppendLine("идентификационен номер, деклариран в предходен данъчен период на изпращане или транспортиране");
                sb.AppendLine("на стоки по чл. 15а, ал. 1 от закона, грешно декларираният идентификационен номер по ДДС се ");
                sb.AppendLine("посочва с код на операция 2, а верният се посочва на отделен ред с код на операция 1;");
                sb.AppendLine("3 - замяна по чл. 15а, ал. 4 от ЗДДС на лицето, за което стоките са предназначени.");
                sb.AppendLine("**В колона к. 5 се вписва данъчният период на изпращането или транспортирането на стоките в");
                sb.AppendLine("случаитe на грешно посочен идентификационен номер, деклариран в предходен данъчен период на ");
                sb.AppendLine("изпращане или транспортиране на стоки по чл. 15а, ал. 1 от закона, или данъчният период, през");
                sb.AppendLine("който е извършена операция, която не е декларирана в периода на извършването ѝ, само когато е  ");
                sb.AppendLine("декларирано първоначално изпращане или транспортиране на стоки от територията на страната ");
                sb.AppendLine("до територията на друга държава членка под режим складиране на стоки до поискване по чл. 15");
                sb.AppendLine("а от ЗДДС.");
                sb.AppendLine();
            }
