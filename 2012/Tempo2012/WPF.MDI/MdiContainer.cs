...
namespace WWPF.MDI
{
	...
	public class MdiContainer : UserControl
	{
        ...
        public static readonly DependencyProperty StatusBarProperty =
            DependencyProperty.Register("StatusBar", typeof(UIElement), typeof(MdiContainer),
            new UIPropertyMetadata(null, new PropertyChangedCallback(StatusBarValueChanged)));
		...
		public MdiContainer()
		{
            ...
            _topPanel.Children.Add(_statusbar = new Border());
		...
		private void Children_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
		{
			switch (e.Action)
			{
				case NotifyCollectionChangedAction.Add:
			        {
			            MdiChild mdiChild =Children[e.NewStartingIndex],topChild = GetTopChild();
        ...
        private void AnimateArangeChildren(MdiChild mdiChild)
        {
            float y = 0;
            foreach (var child in this.Children)
            {
                ...
                child.MainChild = false;
                child.Height = 200;
                child.Width = 200;
                movetoslot(child,new Point(y,this.ActualWidth-200));
                y = y + 200;
            }
        }
		...
		private static void ThemeValueChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e)
		{
			...
			if (currentResourceDictionary != null)
                ...
                {
                    case ThemeType.Luna:
                        Application.Current.Resources.MergedDictionaries.Add(currentResourceDictionary = new ResourceDictionary { Source = new Uri(@"/WWPF.MDI;component/Themes/Luna.xaml", UriKind.Relative) });
                        break;
                    case ThemeType.Aero:
                        Application.Current.Resources.MergedDictionaries.Add(currentResourceDictionary = new ResourceDictionary { Source = new Uri(@"/WWPF.MDI;component/Themes/Aero.xaml", UriKind.Relative) });
                        break;
                }
        }
		...
		#endregion
        #region Animation
        public void AnimateArangeChildren()
        {
	    ...
	    private void movetoslot(MdiChild child,Point point )
	    {
	        var top = child.Position.Y;
	        var left =child.Position.X;
	        TranslateTransform trans = new TranslateTransform();
	        child.RenderTransform = trans;
	        DoubleAnimation anim1 = new DoubleAnimation(top, point.X - top, TimeSpan.FromSeconds(1));
	        DoubleAnimation anim2 = new DoubleAnimation(left, point.Y - left, TimeSpan.FromSeconds(1));
	        trans.BeginAnimation(TranslateTransform.YProperty, anim1);
	        trans.BeginAnimation(TranslateTransform.XProperty, anim2);
	    }
        public void SunSet(MdiChild child)
        {
            if (child.MainChild) return;
            child.MainChild = true;           
            child.Position=new Point(0,0);
            child.Height =this.ActualHeight-20>0?this.ActualHeight-20:900;
            child.Width = this.ActualWidth - 200>0?this.ActualWidth - 200:900;
            ScaleTransform trans = new ScaleTransform();
            child.RenderTransform = trans;
            // if you use the same animation for X & Y you don't need anim1, anim2 
            DoubleAnimation anim = new DoubleAnimation(0.5, 1, TimeSpan.FromMilliseconds(500));
        ...
        internal void ReArrange(MdiChild mdiChild)
        {
            if (!mdiChild.MainChild)
            {
                AnimateArangeChildren(mdiChild);
                SunSet(mdiChild);
            }
        }
    }
